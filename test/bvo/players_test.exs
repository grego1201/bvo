defmodule Bvo.PlayersTest do
  use Bvo.DataCase

  alias Bvo.Players

  describe "players" do
    alias Bvo.Players.Player

    import Bvo.PlayersFixtures

    @invalid_attrs %{name: nil, surname: nil, age: nil}
    ...
    test "create_player/1 with valid data creates a player" do
      valid_attrs = %{name: "some name", surname: "some surname", age: 42}

      assert {:ok, %Player{} = player} = Players.create_player(valid_attrs)
      assert player.name == "some name"
      assert player.surname == "some surname"
    ...
    test "update_player/2 with valid data updates the player" do
      player = player_fixture()
      update_attrs = %{name: "some updated name", surname: "some updated surname", age: 43}

      assert {:ok, %Player{} = player} = Players.update_player(player, update_attrs)
      assert player.name == "some updated name"
      assert player.surname == "some updated surname"
